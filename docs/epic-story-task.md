...
- [x] **Task** 확정/취소 상태 전이 동시성 보강
  - `Enrollment` row에 비관적 락 적용
  - 동일 신청의 `confirm` / `cancel`, `cancel` / `cancel` 동시 실행 시 상태와 `currentEnrollment` 정합성 검증
...
- [x] **Task** 페이지네이션 적용 (내 수강 신청 목록, 강의 목록 조회, 강의별 확정 수강생 목록 API)
...
- [x] **Task** 엣지 케이스 체크리스트 기반 테스트 작성
  - [x] `PENDING` 상태에서 결제 없이 취소
  - [x] 동일 강의 중복 신청 시도
  - [x] `DRAFT` / `CLOSED` 강의에 신청 시도
  - [x] 정원 1석에 동시 다수 신청 (동시성)
  - [x] 동일 신청에 대한 동시 확정/취소 및 중복 취소 (동시성)
  - [x] 결제 후 정확히 7일째 되는 날 취소 시도 (경계값)
  - [x] 존재하지 않는 강의/신청 ID로 요청

### Story 6-2. 코드 리팩토링

- [x] **Task** `LectureService` 내 Entity → 응답 DTO 매핑을 DTO 정적 팩토리로 이동
- [x] **Task** 코드 포맷팅 도구(Spotless) 적용
- [x] **Task** 코드 스타일 검사 도구(Checkstyle) 적용

### Story 6-3. README 문서화 완성
