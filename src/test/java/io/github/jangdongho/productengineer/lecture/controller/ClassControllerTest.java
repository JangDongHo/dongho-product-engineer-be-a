...
class ClassControllerTest {
  ...
  void getList_all() throws Exception {
    ...
    ClassListItemResponse item =
        new ClassListItemResponse(
            1L,
            10L,
            "A",
            ClassStatus.DRAFT,
            ...
            LocalDateTime.parse("2026-05-30T18:00:00"));
  ...
  void getList_invalidPagination_returns400() throws Exception {
    mockMvc
        .perform(get("/classes").param("page", "-1").param("size", "101"))
  ...
  void getList_pageExceedsMax_returns400() throws Exception {
    mockMvc
        .perform(get("/classes").param("page", "10001").param("size", "20"))
        .andExpect(status().isBadRequest())
        .andExpect(jsonPath("$.code", is(ErrorCode.VALIDATION_ERROR.getCode())));
  }
  ...
  void getById_returnsDetail() throws Exception {
    ClassDetailResponse body =
        new ClassDetailResponse(
            ...
            LocalDateTime.parse("2026-07-31T18:00:00"));
    when(lectureService.getClassById(1L)).thenReturn(body);

    mockMvc
        .perform(get("/classes/1"))
        .andExpect(status().isOk())
  ...
  void getById_notFound_returns404() throws Exception {
    when(lectureService.getClassById(99L)).thenThrow(new BusinessException(ErrorCode.NOT_FOUND));

    mockMvc
        .perform(get("/classes/99"))
  ...
  void getEnrollments_confirmedList_returns200() throws Exception {
    PageRequest pageable = PageRequest.of(0, 20);
    ClassConfirmedEnrollmentItemResponse row =
        new ClassConfirmedEnrollmentItemResponse(
    ...
    mockMvc
        .perform(get("/classes/1/enrollments").param("creatorId", "10"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.success", is(true)))
  ...
  void getEnrollments_empty_returns200() throws Exception {
    ...
    mockMvc
        .perform(get("/classes/2/enrollments").param("creatorId", "1"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.success", is(true)))
  ...
  void getEnrollments_notFound_returns404() throws Exception {
    ...
    mockMvc
        .perform(get("/classes/99/enrollments").param("creatorId", "1"))
        .andExpect(status().isNotFound())
        .andExpect(jsonPath("$.code", is(ErrorCode.NOT_FOUND.getCode())));
  }
  ...
  void getEnrollments_forbidden_returns403() throws Exception {
    ...
    mockMvc
        .perform(get("/classes/1/enrollments").param("creatorId", "2"))
  ...
  void getEnrollments_missingCreatorId_returns400() throws Exception {
    mockMvc
        .perform(get("/classes/1/enrollments"))
        .andExpect(status().isBadRequest())
        .andExpect(jsonPath("$.code", is(ErrorCode.VALIDATION_ERROR.getCode())));
  }
  ...
  void getEnrollments_invalidCreatorId_returns400() throws Exception {
    mockMvc
        .perform(get("/classes/1/enrollments").param("creatorId", "-1"))
        .andExpect(status().isBadRequest())
        .andExpect(jsonPath("$.code", is(ErrorCode.VALIDATION_ERROR.getCode())));
  }
  ...
  void patchStatus_returns200() throws Exception {
    ...
    String body = """
				...
				""";
    mockMvc
        .perform(patch("/classes/1/status").contentType(MediaType.APPLICATION_JSON).content(body))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.success", is(true)))
