...
class EnrollmentControllerTest {
  ...
  void enroll_returns201() throws Exception {
    ...
    String body = """
				...
				""";
    mockMvc
        .perform(post("/enrollments").contentType(MediaType.APPLICATION_JSON).content(body))
  ...
  void list_returns200() throws Exception {
    ...
    mockMvc
        .perform(get("/enrollments").param("userId", "1").param("page", "1").param("size", "2"))
  ...
  void list_nonPositiveUserId_returns400() throws Exception {
    mockMvc
        .perform(get("/enrollments").param("userId", "0"))
        .andExpect(status().isBadRequest())
        .andExpect(jsonPath("$.code", is(ErrorCode.VALIDATION_ERROR.getCode())));
  }
  ...
  void list_invalidPagination_returns400() throws Exception {
    mockMvc
        .perform(get("/enrollments").param("userId", "1").param("page", "-1").param("size", "101"))
        .andExpect(status().isBadRequest())
        .andExpect(jsonPath("$.code", is(ErrorCode.VALIDATION_ERROR.getCode())));
  }
  ...
  void list_pageExceedsMax_returns400() throws Exception {
    mockMvc
        .perform(
            get("/enrollments").param("userId", "1").param("page", "10001").param("size", "20"))
        .andExpect(status().isBadRequest())
        .andExpect(jsonPath("$.code", is(ErrorCode.VALIDATION_ERROR.getCode())));
  }
  ...
  void list_lectureMissing_returns404() throws Exception {
    ...
    mockMvc
        .perform(get("/enrollments").param("userId", "1"))
        .andExpect(status().isNotFound())
        .andExpect(jsonPath("$.code", is(ErrorCode.NOT_FOUND.getCode())));
  }
  ...
  void confirm_returns200() throws Exception {
    ...
    mockMvc
        .perform(patch("/enrollments/1/confirm"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.success", is(true)))
    ...
    when(enrollmentService.confirm(2L))
        .thenThrow(
            new BusinessException(
                ErrorCode.VALIDATION_ERROR, "결제 대기(PENDING) 상태의 신청만 확정할 수 있습니다."));

    mockMvc
        .perform(patch("/enrollments/2/confirm"))
        .andExpect(status().isBadRequest())
        .andExpect(jsonPath("$.code", is(ErrorCode.VALIDATION_ERROR.getCode())));
  }
  ...
  void confirm_notFound_returns404() throws Exception {
    when(enrollmentService.confirm(99L)).thenThrow(new BusinessException(ErrorCode.NOT_FOUND));

    mockMvc
        .perform(patch("/enrollments/99/confirm"))
        .andExpect(status().isNotFound())
        .andExpect(jsonPath("$.code", is(ErrorCode.NOT_FOUND.getCode())));
  }
  ...
  void cancel_returns200() throws Exception {
    ...
    mockMvc
        .perform(patch("/enrollments/1/cancel"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.success", is(true)))
        .andExpect(jsonPath("$.data.id", is(1)))
  ...
  void cancel_validationError_returns400() throws Exception {
    ...
    mockMvc
        .perform(patch("/enrollments/2/cancel"))
        .andExpect(status().isBadRequest())
        .andExpect(jsonPath("$.code", is(ErrorCode.VALIDATION_ERROR.getCode())));
  }
  ...
  void cancel_notFound_returns404() throws Exception {
    when(enrollmentService.cancel(99L)).thenThrow(new BusinessException(ErrorCode.NOT_FOUND));
